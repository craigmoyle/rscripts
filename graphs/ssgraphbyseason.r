...
SSN_2020 %>% filter(stat=="attempt_from_zone2") %>% group_by(squadName) %>% summarise(Total = sum(value)) %>% arrange(desc(Total)) %>%
...
# Creaate frame with made and attempts
SSN_Attempt <- SSN_2020 %>% filter(stat=="attempt_from_zone2") %>% group_by(squadName) %>% summarise(Attempt = sum(value)) %>% arrange(desc(Attempt))
SSN_Made <- SSN_2020 %>% filter(stat=="goal_from_zone2") %>% group_by(squadName) %>% summarise(Made = sum(value)) %>% arrange(desc(Made))
SS_All <- left_join(SSN_Made, SSN_Attempt, by = "squadName")
